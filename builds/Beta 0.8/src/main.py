...
import json
#Custom Modules
from sceneFile import scene,world,player,blankmesh
import guibuttons
import shapes
...
while running: # Loop while program is running
    ...
    if(not(sim)):
        ...
        if guiEvents[0]["pressed"]: # Increment selection
            ...
            chcs = easygui.choicebox("Shape:","Add New Mesh",["Cube","Pyramid","Octahedron","Sphere","PlaneXZ","PlaneXY","PlaneYZ","SmoothCube","OBJ"])
            if chcs != None:
                if chcs == 'OBJ':
                    fnam = easygui.fileopenbox(filetypes=['*.obj','*.txt','*.*'])
                    if fnam != None:
                        try:
                            f = open(fnam,'r')
                            blankm['shape'] = loadobj(f.read())
                            f.close()
                            blankm['name'] = fnam
                            scene.append(blankm)
                        except:

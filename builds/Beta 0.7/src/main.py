...
def gui(mouseX,mouseY,mousePressed,msh,idx): # Function for dealing with GUI
    nams = ["Cube","Pyramid","Octahedron","Sphere","PlaneXZ","PlaneXY","PlaneYZ","CubeSmooth"] # Names of shapes
...
while running: # Loop while program is running
    ...
    if(not(sim)):
        ...
        if guiEvents[0]["pressed"]: # Increment selection
            ...
            sel%=len(scene)
            sleep(0.1)
        elif guiEvents[1]["pressed"]: # Increment mesh shape
            scene[sel]["shape"]+=1
